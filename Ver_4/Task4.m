...
for cnt = 1:N
    ...
    subplot(2,1,2) %subplotting
    imshow(Starexract == cnt); %%showing the subplots through for loop
    
    z = findpeaks(r,'MinPeakProminence',5)
    subplot(2,1,1); %%finding the peak that only is a height of 5 
    plot(th,r,'.');

    title(length(z)); %%syas how many plots there are 
    
    if length(z) == 4 %% this is the values for the for loop 
        newimg = newimg + (Starexract == cnt); %%assumintg that it has 4 peaks
    end
end
