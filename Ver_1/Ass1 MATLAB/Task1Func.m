...
[ newCols, newRows ] = meshgrid ( 1 : transCols , 1 : transRows ); % meshgrid returns a two dimensional array.
...
fractR = newRows * scaleR; % newRows could be called Jrows (Also Rout)
fractC = newCols * scaleC;  
...
% Process behind the weighting?
% Conditions REF Slide 58 "Adjusting indices"
zeroR ( zeroR < 1 ) = 1; 
zeroC ( zeroC < 1 ) = 1; 
zeroR ( zeroR > indexRows - 1 ) = indexRows - 1; 
zeroC ( zeroC > indexCols - 1 ) = indexCols - 1; 

% This is the same as the NN scaling ^^^^
